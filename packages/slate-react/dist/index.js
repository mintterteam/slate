...
function verifyDiffState(editor, textDiff) {
  ...
  if (diff.start !== node.value.length || diff.text.length === 0) {
    return node.value.slice(diff.start, diff.start + diff.text.length) === diff.text;
  }
  ...
  return slate.Text.isText(nextNode) && nextNode.value.startsWith(diff.text);
}
...
function createAndroidInputManager(_ref) {
  ...
  var storeDiff = function storeDiff(path, diff) {
    ...
    if (idx < 0) {
      var normalized = normalizeStringDiff(target.value, diff);
    ...
    var merged = mergeStringDiffs(target.value, pendingDiffs[idx].diff, diff);
